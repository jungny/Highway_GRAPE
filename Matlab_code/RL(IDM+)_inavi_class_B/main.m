        ...
        if mod(int32(Time/Parameter.Physics),int32(Parameter.Control/Parameter.Physics)) == 0
            for i = 1:size(List.Vehicle.Active,1)
                if V2V_cancel(List.Vehicle.Active(i,1)) == false
                    ...
                    elseif ~isempty(List.Vehicle.Object{List.Vehicle.Active(i,1)}) &&  List.Vehicle.Object{List.Vehicle.Active(i,1)}.Location > Trajectory_len(2) - 7500 %7300이 교차로 통과 지점인듯!
            ...
            for i = 1:size(List.Vehicle.Active,1)
                %priority eval
                if ~isempty(List.Vehicle.Object{List.Vehicle.Active(i,1)}) &&  ~isempty(V2V.Object{List.Vehicle.Active(i,1)}) && List.Vehicle.Object{List.Vehicle.Active(i,1)}.Location>1000 && V2V_cancel(i) == false 
                    V2V.Object{List.Vehicle.Active(i,1)} = GetReserve(V2V.Object{List.Vehicle.Active(i,1)},Receive_V2V{List.Vehicle.Active(i,1)},List.Vehicle.Object{List.Vehicle.Active(i,1)});
                    %List.Vehicle.Object{List.Vehicle.Active(i,1)}.Velocity = V2V.Object{List.Vehicle.Active(i,1)}.vel;
                ...
                for j = 1 : size(List.Vehicle.Active,1)
                    if List.Vehicle.Active(j,1) ~= List.Vehicle.Active(i,1) && List.Vehicle.Object{List.Vehicle.Active(i,1)}.Location > 1000
                        if ~isempty(V2V.Object{List.Vehicle.Active(j,1)})
                            Receive_V2V{List.Vehicle.Active(i,1)}(end+1, :) = ReceiveMsg(V2V.Object{List.Vehicle.Active(j,1)});
                            Receive_V2V_check(List.Vehicle.Active(i,1)) = true;
                        end
                    end
                end
            end

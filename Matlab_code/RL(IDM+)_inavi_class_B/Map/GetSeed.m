function Seed = GetSeed(Settings,Parameter,Iteration)
    ...
    if Settings.Mode == 1
        Seed(1,:) = [3,2,1]%1:TotalVehicles;        
        Seed(2,:) = [1, 1.3, 1.8];%((randperm(21,TotalVehicles)-1)*Parameter.Physics);            
        Seed(3,:) = [3,2,1];%randi([1,4],[1,TotalVehicles]);    
        if Settings.Iterations(2,Iteration) == 1
            ...
            Seed(4,:) = [2, 1, 1];%randi([1,3],[1,TotalVehicles]);
        end
